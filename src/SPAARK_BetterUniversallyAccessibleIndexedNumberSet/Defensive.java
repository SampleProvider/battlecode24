...
public class Defensive {
    ...
    protected static void run() throws GameActionException {
        ...
        if (!hasFoundFlag) {
            MapLocation targetLoc = UniversallyAccessibleIndexedNumberSet.parseLocation(rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_CUR_LOC + (UniversallyAccessibleIndexedNumberSet.id % 3)));
            ...
            if (me.distanceSquaredTo(targetLoc) <= 2) {
                ...
                if (flags.length > 0) {
                    for (FlagInfo flag : flags) {
                        if (flag.getLocation().equals(targetLoc) && flag.getID() == rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_ID + (UniversallyAccessibleIndexedNumberSet.id % 3))) {
                            rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_DEF_LOC + (UniversallyAccessibleIndexedNumberSet.id % 3), rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_CUR_LOC + (UniversallyAccessibleIndexedNumberSet.id % 3)));
                            break;
                        }
                    }
                }
        ...
        if (UniversallyAccessibleIndexedNumberSet.hasLocation(rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_DEF_LOC + UniversallyAccessibleIndexedNumberSet.id))) {
            ...
            if (opponentRobots.length > 0) {
                ...
                MapLocation targetLoc = UniversallyAccessibleIndexedNumberSet.parseLocation(rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_DEF_LOC + (UniversallyAccessibleIndexedNumberSet.id % 3)));
                rc.setIndicatorLine(me, targetLoc, 255, 0, 255);
                if (UniversallyAccessibleIndexedNumberSet.id < 3) {
        ...
        UniversallyAccessibleIndexedNumberSet.updateSector();
