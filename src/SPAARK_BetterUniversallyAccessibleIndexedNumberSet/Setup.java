...
public class Setup {
    ...
    protected static Boolean pickupFlag() throws GameActionException {
        ...
        int flagtarget = rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_FLAG_TARGET);
        if (closestFlag != null && rc.canPickupFlag(closestFlag.getLocation())) {
            ...
            boolean found = false;
            for (int i = 0; i <= 2; i++) {
                if (rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_ID + i) == closestFlag.getID()) {
                    flagIndex = i;
                    found = true;
                }
            }
            if (found) {
                rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_FLAG_TARGET, flagtarget);
                rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_DEF_LOC + flagIndex, UniversallyAccessibleIndexedNumberSet.intifyLocation(flagInit));
            }
            return found;
        }
        return false;
    ...
    protected static void moveFlag() throws GameActionException {
        MapLocation flagTarget = UniversallyAccessibleIndexedNumberSet.parseLocation(rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_FLAG_TARGET));
        ...
        if (rc.canSenseLocation(toPlace)) {
            ...
            for (int i = 0; i <= 2; i++) {
                ...
                if (UniversallyAccessibleIndexedNumberSet.hasLocation(n) && !UniversallyAccessibleIndexedNumberSet.isFlagPickedUp(n)) {
                    if (toPlace.distanceSquaredTo(UniversallyAccessibleIndexedNumberSet.parseLocation(n)) < 36) {
                        valid = false;
                    }
                }
            }
        ...
        if (rc.canDropFlag(toPlace)) {
            rc.dropFlag(toPlace);
            rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_CUR_LOC + flagIndex, UniversallyAccessibleIndexedNumberSet.intifyLocation(toPlace));
            flagIndex = -1;
        }
        else {
            ...
            rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_CUR_LOC + flagIndex, (7 << 13) | UniversallyAccessibleIndexedNumberSet.intifyLocation(rc.getLocation()));
        }
    }
    ...
    protected static void run() throws GameActionException {
        if (rc.getRoundNum() < Math.max(rc.getMapHeight(), rc.getMapWidth())) {
            ...
            if (!rc.hasFlag()) {
                ...
                for (MapInfo i : infos) {
                    if (i.isDam()) {
                        ...
                        int meet = rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_GATHER_LOC);
                        if (!UniversallyAccessibleIndexedNumberSet.hasLocation(meet)) {
                            rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_GATHER_LOC, UniversallyAccessibleIndexedNumberSet.intifyLocation(damInit));
                        }
                    }
                }
            }
            ...
            if (damInit == null) {
                ...
                int best = rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_FLAG_WEIGHT);
                if (best < weight) {
                    rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_FLAG_WEIGHT, weight);
                    rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_FLAG_TARGET, UniversallyAccessibleIndexedNumberSet.intifyLocation(rc.getLocation()));
                }
            }
            ...
            if (rc.hasFlag()) {
                moveFlag();
            } else if (UniversallyAccessibleIndexedNumberSet.id < 6) {
                MapLocation flagCarrier = UniversallyAccessibleIndexedNumberSet.parseLocation(rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.ALLY_FLAG_CUR_LOC + (UniversallyAccessibleIndexedNumberSet.id % 3)));
            ...
            if (rc.hasFlag()) {
                ...
                Motion.bugnavTowards(flagCarrier);
                rc.setIndicatorLine(rc.getLocation(), flagCarrier, 255, 0, 255);
                indicatorString.append("FOLLOW-FLAG;" + UniversallyAccessibleIndexedNumberSet.id);
            } else {
                MapInfo[] info = rc.senseNearbyMapInfos();
                int damLoc = rc.readSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_GATHER_LOC);
                if (!UniversallyAccessibleIndexedNumberSet.hasLocation(damLoc)) {
                    for (MapInfo i : info) {
                        if (i.isDam()) {
                            rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_GATHER_LOC, UniversallyAccessibleIndexedNumberSet.intifyLocation(i.getMapLocation()) | 1 << 13);
                            break;
                        }
                    }
                }
                ...
                if (!nearDam) {
                    ...
                    if (UniversallyAccessibleIndexedNumberSet.hasLocation(damLoc)) {
                        Motion.bugnavTowards(UniversallyAccessibleIndexedNumberSet.parseLocation(damLoc), 500);
                        indicatorString.append("MEET("+UniversallyAccessibleIndexedNumberSet.parseLocation(damLoc).x+","+UniversallyAccessibleIndexedNumberSet.parseLocation(damLoc).y+");");
                        rc.setIndicatorLine(me, UniversallyAccessibleIndexedNumberSet.parseLocation(damLoc), 255, 100, 0);
                    } else {
                        Motion.moveRandomly();
                    ...
                    if (!UniversallyAccessibleIndexedNumberSet.hasLocation(damLoc) || botWeight > storedBotWeight) {
                        //Changing meeting point if there are lots of enemy bots/not many friendly bots
                        for (MapInfo i : info) {
                            if (i.isDam()) {
                                rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_GATHER_LOC, UniversallyAccessibleIndexedNumberSet.intifyLocation(i.getMapLocation()) | botWeight << 13);
                                break;
                            }
                        }
                    }
                    if (me.distanceSquaredTo(UniversallyAccessibleIndexedNumberSet.parseLocation(damLoc)) < 5) {
                        rc.writeSharedArray(UniversallyAccessibleIndexedNumberSet.SETUP_GATHER_LOC, UniversallyAccessibleIndexedNumberSet.intifyLocation(UniversallyAccessibleIndexedNumberSet.parseLocation(damLoc)) | botWeight << 13);
                    }
                    for (MapInfo i : info) {
                        if (i.isDam()) {
        ...
        if (UniversallyAccessibleIndexedNumberSet.id < 6) {
