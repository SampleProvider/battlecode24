...
public class UniversallyAccessibleIndexedNumberSet {
    ...
    */
    ...
    protected static MapLocation getGroupTarget(int index) throws GameActionException {
        ...
        if (UniversallyAccessibleIndexedNumberSet.isGlobalArrayLoc(n)) {
            ...
            if (!UniversallyAccessibleIndexedNumberSet.hasLocation(n2)) {
            ...
            return UniversallyAccessibleIndexedNumberSet.parseLocation(n2);
        }
        else {
            return UniversallyAccessibleIndexedNumberSet.parseLocation(n);
        }
    }
    ...
    protected static void allocateGroups() throws GameActionException {
        if (rc.getRoundNum() % 2 == 0) {
            ...
            for (int i = 0; i <= 2; i++) {
                ...
                if (UniversallyAccessibleIndexedNumberSet.isFlagInDanger(n) && UniversallyAccessibleIndexedNumberSet.hasLocation(n)) {
                    int d = rc.readSharedArray(ALLY_FLAG_INFO + i);
                    // int number = GlobalArray.getNumberOfRobots(d) - friendlyFlagGroupsAssigned[i] * 5;
                    int number = UniversallyAccessibleIndexedNumberSet.getNumberOfRobots(d);
                    for (int j = 0; j < 8; j++) {
