...
public class Build {
    ...
    protected static void spamWater() throws GameActionException {
        //spam water to level up
        if (rc.getLevel(SkillType.BUILD) >= 6 || rc.getCrumbs() < 1000) {
            return;
        }
        for (Direction d : DIRECTIONS) {
            if (rc.canDig(rc.getLocation().add(d)) && (rc.getLocation().add(d).x + rc.getLocation().add(d).y) % 2 == 0) {
