    ...
    protected static void run() throws GameActionException {
        ...
        if (Comms.hasLocation(rc.readSharedArray(Comms.ALLY_FLAG_DEF_LOC + Comms.id))) {
            ...
            if (opponentRobots.length > 0) {
                ...
                rc.setIndicatorLine(me, targetLoc, 255, 0, 255);
                if (Comms.id < 3) {
                    Motion.bugnavAround(Comms.getCloserToAxis(targetLoc, 2), 0, 8, 0, false);
                    me = rc.getLocation();
                    ...
                    if (flags.length > 0) {
                        for (int j = 0; j < 8; j++) {
                            MapLocation buildLoc = me.add(DIRECTIONS[j]);
                            if ((buildLoc.x + buildLoc.y) % 2 == 0 && buildLoc.distanceSquaredTo(Comms.getCloserToAxis(targetLoc, 2)) < 10) {
                                if (rc.canDig(buildLoc)) rc.dig(buildLoc);
                                continue;
                            } else {
                                // if (rc.canBuild(TrapType.STUN, buildLoc)) {
                                //     rc.build(TrapType.STUN, buildLoc);
