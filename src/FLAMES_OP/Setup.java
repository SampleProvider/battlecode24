            ...
            if ((rc.getRoundNum() % 40 == 0 || flagTarget.x < 0 || rc.getLocation().equals(flagTarget))) {
                ...
                t: for (MapInfo i : mapInfos) {
                    ...
                    for (int j = 3; --j >= 0;) {
                        ...
                        weight -= Math.sqrt(Math.sqrt(rc.getMapHeight() * rc.getMapWidth()) * (Math.sqrt(me.distanceSquaredTo(allFlags[j])) / Math.sqrt(i.getMapLocation().distanceSquaredTo(allFlags[j]))));
